...
public class MiniAdminSwitch {
    ...
    private static void borrarSeguro(File base, String rutaRel) throws IOException {
        ...
        try {
            if (objetivo.delete()) {
                System.out.println("Borrado exitoso de: " + objetivo.getPath());
            } else {
                throw new IOException("No se pudo borrar");
            }
        } catch (SecurityException e) {
            throw new IOException("No se pudo borrar por restricciones de seguridad", e);
        } finally {
    ...
    private static void renombrarSeguro(File base, String origenRel, String destinoRel) {
        ...
        if (!origen.exists()) {
            System.err.println("El archivo origen no existe: " + origen.getPath());
            return;
        }
        if (destino.exists()) {
            System.err.println("El archivo destino ya existe: " + destino.getPath());
            return;
        }
        File padreDestino = destino.getParentFile();
        if (padreDestino != null && !padreDestino.exists()) {
            System.err.println("El directorio padre del destino no existe: " + padreDestino.getPath());
            return;
        }
        try {
            if (origen.renameTo(destino)) {
                System.out.println("Renombrado exitoso de: " + origen.getPath() + " a " + destino.getPath());
            } else {
                System.err.println("No se pudo renombrar el archivo");
            }
        } catch (SecurityException e) {
            System.err.println("No se pudo renombrar por restricciones de seguridad: " + e.getMessage());
        } finally {
    ...
    private static void cambiarPermiso(File base, String rutaRel, String permiso, boolean activar) {
        ...
        if (!f.exists()) {
            System.err.println("El archivo no existe: " + f.getPath());
            return;
        }
        try {
            boolean resultado;
            switch (permiso) {
                case "lectura":
                    resultado = f.setReadable(activar, false);
                    break;
                case "escritura":
                    resultado = f.setWritable(activar, false);
                    break;
                case "ejecucion":
                    resultado = f.setExecutable(activar, false);
                    break;
                default:
                    System.err.println("Permiso no válido: " + permiso);
                    return;
            }
            if (resultado) {
                System.out.println("Permiso " + permiso + " cambiado a " + (activar ? "on" : "off") + " para: " + f.getPath());
            } else {
                System.err.println("No se pudo cambiar el permiso " + permiso + " para: " + f.getPath());
            }
        } catch (SecurityException e) {
            System.err.println("No se pudo cambiar el permiso por restricciones de seguridad: " + e.getMessage());
        } finally {
            System.out.println("[permiso] Fin");
        }
